...
const create = (userId, items, totalAmount) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        yield orderItems_schema_1.orderItemsModel.bulkCreate(orderItems, { transaction });
        const inventoryUpdates = items.map((item) => ({
            groceryId: item.groceryId,
            quantity: item.quantity,
        }));
        const updatePromises = inventoryUpdates.map((update) => utility_1.sequelize.query(`UPDATE "inventories" SET quantity = quantity - :quantity WHERE "groceryId" = :groceryId`, {
            replacements: {
